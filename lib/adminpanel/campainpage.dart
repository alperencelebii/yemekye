...
class _CampaignPageState extends State<CampaignPage> {
  ...
  void _createCampaign(String productId, String productName, {Map<String, dynamic>? existingCampaign}) {
  ...
  Future<void> _endCampaign(String campaignId) async {
    DocumentSnapshot campaignDoc = await _firestore.collection('campaigns').doc(campaignId).get();
    if (campaignDoc.exists) {
      Map<String, dynamic> campaignData = campaignDoc.data() as Map<String, dynamic>;
                    ...
                    builder: (context, AsyncSnapshot<DocumentSnapshot> productSnapshot) {
                      ...
                      return StreamBuilder(
                        stream: _firestore
                            ...
                            .snapshots(),
                        builder: (context, AsyncSnapshot<QuerySnapshot> campaignSnapshot) {
                          bool isActive = false;
                          String? campaignId;
                          Timestamp? startTime;
                          Timestamp? endTime;

                          if (campaignSnapshot.hasData && campaignSnapshot.data!.docs.isNotEmpty) {
                            var campaignData = campaignSnapshot.data!.docs.first;
                            DateTime now = DateTime.now();
                            campaignId = campaignData.id;
                            startTime = campaignData['start_time'];
                            endTime = campaignData['end_time'];
                            isActive = now.isAfter(startTime!.toDate()) && now.isBefore(endTime!.toDate());
                          }
